...
class FRFTAudioProcessor extends AudioWorkletProcessor {
    ...
    _processFrame() {
        ...
        // Pre-apply fftshift to the windowed frame so the engine's internal
        // fftshift cancels it: engine sees win*frame in natural order.
        const half = N >> 1;
        for (let i = 0; i < N; i++) {
            this._heapInR[(i + half) % N] = this._frame[i] * this._win[i];
        }
        this._heapInI.fill(0);

        // α ≡ 0 (mod 4) → identity bypass.
        const amod = ((this._alpha % 4) + 4) % 4;
        const isId = amod < 1e-9 || amod > 4 - 1e-9;

        if (isId) {
            for (let i = 0; i < N; i++) {
                this._ola[i] += this._heapInR[(i + half) % N] * this._win[i];
            }
        } else {
            // FRFT

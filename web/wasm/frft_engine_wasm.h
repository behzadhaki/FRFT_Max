...
// Use double precision with KissFFT
...
private:
    // Core algorithm functions (identical to original)
    ...
    // KissFFT plan cache
    struct PlanCache {
        kiss_fft_cfg forward_cfg = nullptr;
        kiss_fft_cfg backward_cfg = nullptr;
        std::vector<kiss_fft_cpx> in_buf;
        std::vector<kiss_fft_cpx> out_buf;
        size_t size = 0;
    };

...
ARCH=$(uname -m)
if [ "$ARCH" != "arm64" ]; then
    echo "ERROR: FRFT is only available for Apple Silicon (arm64)."
    echo "       Your architecture is: $ARCH"
    exit 1
fi

ZIP_URL="https://github.com/behzadhaki/FRFT_Max/releases/download/v1.0/FRFT-osx-arm64.zip"
...
curl -L --progress-bar "$ZIP_URL" -o "$TMP_DIR/FRFT.zip"
unzip -q "$TMP_DIR/FRFT.zip" -d "$TMP_DIR"

# Remove existing installation if present
rm -rf "$MAX_PACKAGES/FRFT"

cp -r "$TMP_DIR/FRFT" "$MAX_PACKAGES/FRFT"

# ── Remove quarantine ──────────────────────────────────────────────────────────
xattr -dr com.apple.quarantine "$MAX_PACKAGES/FRFT"

echo ""
echo "FRFT installed successfully at:"
echo "  $MAX_PACKAGES/FRFT"

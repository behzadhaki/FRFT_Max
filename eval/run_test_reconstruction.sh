#!/bin/bash
# Shell script to compile and run frft_test_reconstruction (macOS only)

set -e  # Exit on error

echo "🔨 Compiling frft_test_reconstruction..."
...
clang++ -std=c++17 -O3 -march=native -ffast-math \
    ...
    "$EVAL_DIR/frft_test_reconstruction.cpp" \
    "$FFTW3_LIB" \
    -lm \
    -o "$EVAL_DIR/frft_test_reconstruction"
...
if [ $? -eq 0 ]; then
    ...
    echo "🚀 Running test..."
    echo "================================================"
    "$EVAL_DIR/frft_test_reconstruction"

    # Show where output files are
    if [ -f "$EVAL_DIR/frft_test_results.txt" ]; then
        echo ""
        echo "📊 Results saved to: eval/frft_test_results.txt"
    fi
    if [ -f "$EVAL_DIR/frft_timing_benchmarks.txt" ]; then
        echo "⏱️  Timing results saved to: eval/frft_timing_benchmarks.txt"
    fi
else

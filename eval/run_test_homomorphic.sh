#!/bin/bash
# Shell script to compile and run frft_test_homomorphic (macOS only)

set -e  # Exit on error

echo "🔨 Compiling frft_test_homomorphic..."
...
clang++ -std=c++17 -O3 -march=native -ffast-math \
    ...
    "$EVAL_DIR/frft_test_homomorphic.cpp" \
    "$FFTW3_LIB" \
    -lm \
    -o "$EVAL_DIR/frft_test_homomorphic"
...
if [ $? -eq 0 ]; then
    ...
    echo "🚀 Running test..."
    echo "================================================"
    "$EVAL_DIR/frft_test_homomorphic"
else

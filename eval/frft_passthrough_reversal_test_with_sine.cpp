...
struct TestConfig {
    ...
    // Sine frequency range for random generation
    ...
    int num_saved_pngs = 3;          // Only save 3 random PNGs
    std::string base_dir = "test_results/passthrough_reversal_with_sine";
    bool use_hamming_window = false; // Apply Hamming window to input signal
};
...
void generate_sine(std::vector<double>& signal, int size, double frequency,
                   double sample_rate) {
    signal.resize(size);
...
void generate_plot_script(const std::string& csv_filename,
    ...
    script << "axes[0].set_title('Original Signal (Sine)', fontsize=12, fontweight='bold')\n";
...
void test_single_frame(FRFTEngine& engine,
    ...
    double sine_freq = freq_dist(gen);

    // Generate sine signal with random frequency
    std::vector<double> original_frame;
    generate_sine(original_frame, window_size, sine_freq, config.sample_rate);

    // Apply Hamming window
    if (config.use_hamming_window) {
    ...
    std::cout << "  Frame " << frame_num << "/" << config.num_test_frames
              << " [" << test_name << ", α=" << alpha
              << ", sine:" << std::fixed << std::setprecision(0) << sine_freq
              << "Hz]: ";
...
void run_test_suite(const TestConfig& config) {
    ...
    std::cout << "  Sine Range: " << config.sawtooth_f_min << " Hz → " << config.sawtooth_f_max << " Hz (random per frame)\n";
    ...
    for (TestType test_type : test_types) {
        ...
        if (results_file.is_open()) {
            ...
            results_file << "  Sine Range: " << config.sawtooth_f_min << " Hz → "
